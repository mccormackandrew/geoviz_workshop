...
We can also add a legend to the plot using the `addLegend()` function:
...
These are the basics of working with Shapefiles in leaflet. There are many more things that can be done with `leaflet` that were not covered in this workshop, such as adding markers, popup boxes, and Shiny integration. To learn more I suggest you visit the [Leaflet for R](https://rstudio.github.io/leaflet/) guide. In addition, [this post](https://allthisblog.wordpress.com/2016/10/12/r-311-with-leaflet-tutorial/) provides an overview of how to use `leaflet` to create and publish web maps.
...
While putting together this workshop, I consulted a number of sources. You may find some of these reasources helpful when working with your own 
```{r eval = FALSE}
